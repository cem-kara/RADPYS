...
Projede emoji kullanılmaz. Tüm ikonlar `ui/icons.py` üzerinden
**qtawesome mdi6** (Material Design Icons) ile gelir.

### Kullanım

```python
from ui.icons import ic, Icon, pixmap

# QPushButton / QAction için QIcon
btn.setIcon(ic("kaydet"))                          # varsayılan renk
btn.setIcon(ic("kaydet", "white", 16))             # renk + boyut
btn.setIconSize(QSize(16, 16))

# QLabel üzerine ikon (pixmap)
lbl = Icon.label("bildirim", T.text2, 18)          # QLabel döner

# Direkt pixmap
lbl.setPixmap(pixmap("uyari_kirmizi", T.red, 14))

# Butona ikon ata (kısayol)
Icon.btn(btn, "duzenle", T.text2, 14)
```

### Mevcut ikon isimleri

| İsim | Kullanım |
|---|---|
| `dashboard` | Sidebar — Dashboard |
| `personel` | Sidebar — Personel |
| `izin` | Sidebar — İzin Takip |
| `saglik` | Sidebar — Sağlık |
| `dozimetre` | Sidebar — Dozimetre |
| `cihaz` | Sidebar — Cihaz |
| `ariza` | Sidebar — Arıza |
| `bakim` | Sidebar — Bakım |
| `rke` | Sidebar — RKE |
| `nobet` | Sidebar — Nöbet |
| `mesai` | Sidebar — Mesai |
| `dokumanlar` | Sidebar — Dökümanlar |
| `rapor` | Sidebar — Raporlar |
| `ayarlar` | Sidebar — Yönetim |
| `bildirim` | Topbar bildirim zili |
| `ara` | Arama kutusu |
| `kullanici` | Kullanıcı avatarı |
| `ekle` | + Yeni buton |
| `duzenle` | Düzenle butonu |
| `kaydet` | Kaydet butonu |
| `sil` | Silme butonu |
| `iptal` / `kapat` | İptal / kapat |
| `geri` | Geri navigasyon |
| `yenile` | Yenile butonu |
| `alert_hata` | AlertBar danger |
| `alert_uyari` | AlertBar warning |
| `alert_basari` | AlertBar success |
| `alert_bilgi` | AlertBar info |
| `kimlik` | Personel kimlik sekmesi |
| `belge` | Belgeler sekmesi |
| `fhsz` | FHSZ sekmesi |

### Yeni ikon eklemek

`ui/icons.py` içindeki `IKONLAR` sözlüğüne tek satır:
```python
"yeni_ikon": "mdi6.icon-name",
```
mdi6 ikon adını https://pictogrammers.com/library/mdi/ adresinden bul.
...
QLabel("🔧")
QPushButton("💾 Kaydet")
self._tabs.addTab(widget, "📅 İzin")

# ✅ Bunun yerine
Icon.label("ariza", T.text2, 16)
btn.setIcon(ic("kaydet", "white", 14)); btn.setText("Kaydet")
self._tabs.addTab(widget, ic("izin", T.text2, 15), "İzin")
```
